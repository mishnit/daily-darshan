...
def test_rejected_payment_review_commits_before_acknowledgement_reply(app_client, monkeypatch):
    ...
    main._process_best_effort_batch(
        c,
        [_tap_payload("9199", "CTA_PAYMENT_REVIEW", "critical-review")],
        critical=True,
    )

    assert events[:2] == ["commit", "send"]
    assert any(
        row["event"] == "PAYMENT_REVIEW_REQUESTED"
        and row["details"] == payment.reference_id
        for row in c.logs.all()
    )
...
def test_deferred_write_flushes_after_window(tmp_path):
    """A successful push is immediate and a repeated push is idempotent."""
    (tmp_path / "csv").mkdir()
    subs = tmp_path / "csv" / "subs.csv"
    gh = FakeGitHub()
    rs = RepoSync(gh, str(tmp_path), ["csv/subs.csv"], enabled=True)
    subs.write_text("new-subscriber\n")
    assert rs.push("immediate") == ["csv/subs.csv"]
    assert rs.push("repeat") == []
...
def test_quiet_window_across_midnight(tmp_path):
    ...
    gh = FakeGitHub()
    rs = RepoSync(gh, str(tmp_path), ["csv/subs.csv"], enabled=True)
    assert rs.push("any-time") == ["csv/subs.csv"]
